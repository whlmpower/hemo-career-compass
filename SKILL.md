---
name: "hemo-career-compass"
description: "通过多参照系进行职业自我认知诊断。Invoke when user asks about career choice between big tech/SOE/civil service/foreign enterprise/startup, or expresses career confusion."
---

# hemo-career-compass

职业罗盘。通过「多参照系」进行通用职业自我认知诊断，帮助用户深度挖掘内心，完成对未来生活期望的梳理，看清自己到底想要什么。

## 触发条件

当用户表达以下意图时，触发本 Skill：
- 询问「大厂和国企怎么选」
- 询问「大厂和国企怎么选」
- 询问「考公还是去企业」
- 询问「要不要创业」
- 询问「考研还是就业」「考研还是考公」「要不要读研」
- 询问「全日制还是非全日制」「国内读研还是留学」
- 询问「先就业还是先考研」「在职考研」
- 表达「工作稳定但缺乏成长」「体制内被困」「想搞副业」
- 提到「自媒体」「副业」「变现」「个人IP」「side hustle」
- 提到「央企/国企信息部」「技术岗天花板」
- 提到「学历焦虑」「学历贬值」「本科学历不够」
- 表达「不想换工作但想做点别的」
- 表达职业选择迷茫、纠结
- 询问职业规划、未来方向
- 想要了解自己适合什么样的工作
- 提到「央企」「体制内」「互联网」「稳定」「高薪」「创业」「外企」等关键词

## 使用方式

本 Skill 采用**自适应对话式诊断**流程，AI 作为「犀利诊断师」角色，通过多轮苏格拉底式提问，帮助用户完成自我认知深化。

### 核心流程

1. **暖启动 + 信息采集**：统一开场引导用户输出丰富上下文，自然覆盖现状、家庭背景、内在驱动力等关键维度
2. **参照系选择**：根据用户输入自动推荐参照系（大厂/国企、考公、外企、创业、副业主副、教育路径），用户可确认或切换
3. **深层诊断**：自适应展开 6 个心理学维度（大五人格、施恩职业锚、施瓦茨价值观、依恋类型、风险容忍度、家庭锚点）
4. **综合诊断**：整合发现，识别矛盾和盲区，应用动态权重矩阵
5. **报告输出**：经用户确认后，输出「职业自我认知报告」（通用版 + 参照系专属分析）

### 文件结构

```
hemo-career-compass/
├── SKILL.md              # 本文件（AI 核心入口）
├── README.md             # 用户文档，包含安装说明
├── install.sh            # macOS/Linux 一键安装脚本
├── install.bat           # Windows 一键安装脚本
├── docs/
│   └── developer.md      # 开发者详细说明
├── prompts/
│   ├── system.md         # 系统提示词（核心）
│   ├── warmup.md         # 统一暖启动话术 + 后续跟进策略
│   ├── socratic.md       # 苏格拉底式提问话术库
│   └── report.md         # 报告生成模板
├── frameworks/
│   ├── bigfive.md        # 大五人格诊断框架
│   ├── schein.md         # 施恩职业锚框架
│   ├── schwartz.md       # 施瓦茨价值观框架
│   ├── attachment.md     # 依恋类型框架
│   ├── risk-tolerance.md # 风险容忍度框架
│   ├── family-anchor.md  # 家庭锚点框架
│   └── weights.md        # 动态权重矩阵（参照系差异化）
├── referencers/          # 参照系模块（可扩展）
│   ├── bigtech-vs-soe/   # 大厂 vs 国企
│   │   ├── meta.json     # 参照系元数据
│   │   ├── insights.md   # 核心洞察和金句
│   │   ├── outcomes.md   # 出路与风险分析
│   │   └── report-addon.md # 报告专属分析
│   ├── civil-service/    # 考公/体制内
│   │   ├── meta.json
│   │   ├── insights.md
│   │   ├── outcomes.md
│   │   └── report-addon.md
│   ├── foreign-enterprise/ # 外企
│   │   ├── meta.json
│   │   ├── insights.md
│   │   ├── outcomes.md
│   │   └── report-addon.md
│   ├── startup/          # 创业
│   │   ├── meta.json
│   │   ├── insights.md
│   │   ├── outcomes.md
│   │   └── report-addon.md
│   └── side-hustle/      # 副业主副
│       ├── meta.json
│       ├── insights.md
│       ├── outcomes.md
│       └── report-addon.md
│   └── education-vs-career/ # 教育路径 vs 职业路径
│       ├── meta.json
│       ├── insights.md
│       ├── outcomes.md
│       └── report-addon.md
└── references/
    └── insights.md       # 通用金句库和资料洞察
```

### 关键原则

- **不替用户做选择**：只照见，不建议
- **案例优先**：要求用户提供具体经历，而非抽象选项
- **犀利但慈悲**：挑战假设，但不评判
- **自适应轮次**：根据用户回答深度动态决定对话轮次，不固定问题数量
- **多 Session 支持**：支持用户带着新案例回来继续探索

## 平台兼容性

本 Skill 采用标准格式设计，支持跨平台安装：

### 支持的平台

| 平台 | 项目级 | 全局级 (macOS/Linux) | 全局级 (Windows) |
|---|---|---|---|
| Trae Code | `.trae/skills/` | `~/.trae/skills/` | `%userprofile%/.trae/skills/` |
| Codex CLI | `.agents/skills/` 或 `.codex/skills/` | `~/.codex/skills/` | `%userprofile%/.codex/skills/` |
| Claude Code | `.claude/skills/` | `~/.claude/skills/` | `%userprofile%\.claude\skills\` |
| 千问办公 | `.qwen/skills/` | `~/.qwenworkcn/skills/` | `%userprofile%/.qwenworkcn/skills/` |
| WorkBuddy | 不支持 | `~/.workbuddy/skills/` | `C:\Users\<用户名>\.workbuddy\skills\` |

### 安装方式

推荐使用一键安装脚本，详见 `README.md`。

### 兼容性要求

1. **文件读取**：Agent 需要能够读取本地文件系统（`prompts/`、`frameworks/`、`references/` 目录）
2. **多轮对话**：Agent 需要支持上下文保持，能够跨轮次引用之前的对话内容
3. **文件写入**：如果需要支持多 Session，Agent 需要能够写入会话摘要文件

## 版本历史

- v1.6.0 (2026-08-15): 简化暖启动流程，统一为单一开场模板，将信息采集融入暖启动阶段
- v1.5.0 (2026-08-10): 为全部6个参照系补充事实依据，新增 `research/` 目录收录33+权威数据来源，insights.md 增加"关键数据速查"章节，outcomes.md 增加"数据支撑"章节
- v1.4.0 (2026-08-10): 新增 education-vs-career 参照系（教育路径 vs 职业路径），覆盖考研/考公/就业/留学/读博/在职考研等决策场景，新增行业学历地图、ROI测算器、决策树等报告模块
- v1.3.0 (2026-08-10): 新增 family-anchor 诊断维度（家庭锚点），增加越狱信号识别机制，补充副业主副报告模块，增加用户画像标签系统，补充副业变现相关金句
- v1.2.0 (2026-08-10): 新增 side-hustle 参照系（副业主副），扩展触发条件覆盖副业/自媒体场景，新增越狱信号识别机制，新增副业变现模型报告模块
- v1.1.0 (2026-08-09): 扩展支持多参照系（考公、外企、创业），增加动态权重矩阵、价值观冲突检测、压力类型分析、决策风格分析、Mermaid 可视化、伦理边界声明
- v1.0.0 (2026-08-09): 初始版本，包含 5 个心理学维度、自适应对话流程、报告模板、跨平台安装脚本
